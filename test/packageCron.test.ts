...
import { runCron } from '../scripts/package-deduction-cron'
...
async function run() {
  ...
  const count = await runCron()
  if (count !== 1) throw new Error('cron processed wrong count')

  const get = rr('GET', null, { clientId: 'c1' })
  await packageHandler(get.req, get.res)
  if (get.res.body.count !== 0) throw new Error('package not deducted via cron')

  console.log('Cron deducted package')
}

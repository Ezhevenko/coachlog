...
let r = spawnSync(
  ...
  [
    ...
    'test/apiFetch401.test.tsx'
  ],
  { stdio: 'inherit', env }
)
if (r.status !== 0) process.exit(r.status)

r = spawnSync('npx', ['ts-node', '-r', 'tsconfig-paths/register', '--compiler-options', '{"module":"commonjs"}', 'test/clientsByTelegram.test.ts'], { stdio: 'inherit', env })
if (r.status !== 0) process.exit(r.status)
r = spawnSync('npx', ['ts-node', '-r', 'tsconfig-paths/register', '--compiler-options', '{"module":"commonjs"}', 'test/workoutExercises.test.ts'], { stdio: 'inherit', env })
if (r.status !== 0) process.exit(r.status)
